...
var Services = _webexPlugin2.default.extend({
  ...
  collectPreauthCatalog: function collectPreauthCatalog(query) {
    ...
    return this.updateServices({ from: 'limited', query: query });
  },
  ...
  updateCredentialsConfig: function updateCredentialsConfig() {
    ...
    if (idbroker && identity) {
      var _webex$config$credent = this.webex.config.credentials,
          authorizationString = _webex$config$credent.authorizationString,
          authorizeUrl = _webex$config$credent.authorizeUrl;

      // This must be set outside of the setConfig method used to assign the
      // idbroker and identity url values.

      this.webex.config.credentials.authorizeUrl = authorizationString ? authorizeUrl : idbroker.replace(trailingSlashes, '') + '/idb/oauth2/v1/authorize';
  ...
  initServiceCatalogs: function initServiceCatalogs() {
    ...
    .then(function (orgId) {
      return _this5.collectPreauthCatalog({ orgId: orgId });
    }).then(function () {
  ...
  initialize: function initialize() {
    ...
    this.listenToOnce(this.webex, 'ready', function () {
      var supertoken = _this6.webex.credentials.supertoken;
      ...
      if (supertoken && supertoken.access_token) {
        ...
        _this6.collectPreauthCatalog(email ? { email: email } : undefined);
      }
    });
  },
  version: '1.89.1'
