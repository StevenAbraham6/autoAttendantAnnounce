...
MeetingInfoUtil.getParsedUrl = function (link) {
  try {
    ...
    _loggerProxy2.default.logger.warn('Meeting-info:util#getParsedUrl --> unable to parse the URL, error: ' + error);
...
MeetingInfoUtil.getHydraId = function (destination) {
  ...
  if (id && _constants.UUID_REG.test(id)) {
    if (type === _constants._ROOM_) {
      return { room: true, destination: id, cluster: cluster };
    }
    if (type === _constants._PEOPLE_) {
      return { people: true, destination: id, cluster: cluster };
    }
...
MeetingInfoUtil.getSipUriFromHydraPersonId = function (destination, webex) {
  return webex.people.get(destination).then(function (res) {
    ...
    _loggerProxy2.default.logger.error('Meeting-info:util#MeetingInfoUtil.getSipUriFromHydraPersonId --> getSipUriFromHydraPersonId ' + err + ' ');
    throw err;
  });
};
    ...
    return _regenerator2.default.wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          ...
          case 35:
            convoUrl = webex.internal.conversation.getUrlFromClusterId({
          ...
          case 39:
            ...
            _loggerProxy2.default.logger.error('Meeting-info:util#generateOptions --> ' + _context.t0);
            throw _context.t0;
