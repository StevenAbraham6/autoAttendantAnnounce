...
var _statsAnalyzer2 = _interopRequireDefault(_statsAnalyzer);
...
var logRequest = function logRequest(request, _ref) {
  var _ref$header = _ref.header,
      ...
      success = _ref$success === undefined ? '' : _ref$success,
      _ref$failure = _ref.failure,
      failure = _ref$failure === undefined ? '' : _ref$failure;

  _loggerProxy2.default.logger.info(header);

  return request.then(function (arg) {
    _loggerProxy2.default.logger.info(success);

    return arg;
  }).catch(function (error) {
    _loggerProxy2.default.logger.error(failure, error);
    throw error;
  });
};
  ...
  function Meeting(attrs, options) {
    ...
    _this.getMediaStreams = function (mediaDirection) {
      ...
      if (mediaDirection && (mediaDirection.sendAudio || mediaDirection.sendVideo || mediaDirection.sendShare)) {
        if (mediaDirection && mediaDirection.sendAudio && mediaDirection.sendVideo && mediaDirection.sendShare && _bowser2.default.safari) {
          _loggerProxy2.default.logger.warn('Meeting:index#getMediaStreams --> Setting `sendShare` to FALSE, due to complications with Safari');

          mediaDirection.sendShare = false;

          _loggerProxy2.default.logger.warn('Meeting:index#getMediaStreams --> Enabling `sendShare` along with `sendAudio` & `sendVideo`, on Safari, causes a failure while setting up a screen share at the same time as the camera+mic stream');
          _loggerProxy2.default.logger.warn('Meeting:index#getMediaStreams --> Please use `meeting.shareScreen()` to manually start the screen share after successfully joining the meeting');
        }

        return _media2.default.getSupportedDevice({
          sendAudio: mediaDirection.sendAudio,
          sendVideo: mediaDirection.sendVideo
        }).catch(function (error) {
          return _promise2.default.reject(new _media4.default('Given constraints do not match permission set for either camera or microphone', error));
        }).then(function (devicePermissions) {
          return _media2.default.getUserMedia((0, _extends3.default)({}, mediaDirection, {
            ...
            _metrics2.default.sendOperationalMetric(metricName, data, metadata);
            throw new _media4.default('Unable to retrieve media streams');
          });
        });
      }

      return _promise2.default.reject(new _media4.default('At least one of the mediaDirection value should be true'));
    };

    _this.getSupportedDevices = function (_ref2) {
      var _ref2$sendAudio = _ref2.sendAudio,
          sendAudio = _ref2$sendAudio === undefined ? true : _ref2$sendAudio,
          _ref2$sendVideo = _ref2.sendVideo,
          sendVideo = _ref2$sendVideo === undefined ? true : _ref2$sendVideo;
      return _media2.default.getSupportedDevice({ sendAudio: sendAudio, sendVideo: sendVideo });
    };
  ...
  (0, _createClass3.default)(Meeting, [{
    ...
    value: function setUpLocusInfoMediaInactiveListener() {
      ...
      this.locusInfo.on(_constants.EVENTS.DISCONNECT_DUE_TO_INACTIVITY, function (res) {
        ...
        _loggerProxy2.default.logger.error('Meeting:index#setUpLocusInfoMediaInactiveListener --> Meeting disconnected due to inactivity: ' + res.reason);
        _this2.reconnect();
      });
    }
    ...
    value: function getAnalyzerMetricsPrePayload(options) {
      if (options) {
        ...
        if (!event) {
          _loggerProxy2.default.logger.error('Meeting:index#getAnalyzerMetricsPrePayload --> Error [Call Analyzer Event', event || '', ']: invalid identifers or event type! ' + this.correlationId);
    ...
    value: function setUpLocusMediaSharesListener() {
      ...
      this.locusInfo.on(_constants.EVENTS.LOCUS_INFO_UPDATE_MEDIA_SHARES, function (payload) {
        ...
        if (_this8.isSharing && _this8.selfId !== contentId && disposition !== _constants.FLOOR_ACTION.RELEASED) {
          _this8.isSharing = false;
          _this8.updateShare({
            sendShare: false,
            receiveShare: _this8.mediaProperties.mediaDirection.receiveShare
          });
        }

        _this8.members.locusMediaSharesUpdate(payload);
      });
    }
    ...
    value: function setUpLocusUrlListener() {
      ...
      this.locusInfo.on(_constants.EVENTS.LOCUS_INFO_UPDATE_URL, function (payload) {
        _this9.members.locusUrlUpdate(payload);
      });
    }
    ...
    value: function setUpLocusInfoMeetingInfoListener() {
      ...
      this.locusInfo.on(_constants.LOCUSINFO.EVENTS.MEETING_INFO_UPDATED, function (payload) {
        if (payload && payload.info) {
          ...
          if (newCanLock && !_this10.inMeetingActions.canLock) {
            _triggerProxy2.default.trigger(_this10, {
              file: 'meeting/index',
              function: 'setUpLocusInfoMeetingInfoListener'
            }, _constants.EVENT_TRIGGERS.MEETING_ACTIONS_UPDATE, {
              canLock: true,
              canUnlock: false,
              canAssignHost: _this10.inMeetingActions.getCanAssignHost()
            });
          }
          if (newCanUnlock && !_this10.inMeetingActions.canUnlock || !newCanLock) {
            _triggerProxy2.default.trigger(_this10, {
              file: 'meeting/index',
              function: 'setUpLocusInfoMeetingInfoListener'
            }, _constants.EVENT_TRIGGERS.MEETING_ACTIONS_UPDATE, {
          ...
          _this10.inMeetingActions.setCanLock(newCanLock || false);
          _this10.inMeetingActions.setCanUnlock(newCanUnlock || false);
        }
      });
    }
    ...
    value: function setUpLocusInfoMeetingListener() {
      ...
      this.locusInfo.on(_constants.EVENTS.REMOTE_ANSWERED_DECLINED, function (payload) {
        _this12.meetingFiniteStateMachine.remote(payload);
      });
      this.locusInfo.on(_constants.EVENTS.DESTROY_MEETING, function (payload) {
        // if self state is NOT left

        // TODO: Handle sharing and wireless sharing when meeting end
        if (_this12.wirelessShare) {
          if (_this12.mediaProperties.shareTrack) {
            _this12.mediaProperties.shareTrack.onended = null;
            _this12.mediaProperties.shareTrack.stop();
          }
        }
        ...
        if (payload.shouldLeave) {
          ...
          _this12.leave({ reason: payload.reason }).then(function () {
            ...
            _loggerProxy2.default.logger.error('Meeting:index#setUpLocusInfoMeetingListener --> DESTROY_MEETING. Issue with leave for meeting, meeting still in collection: ' + _this12.meeting + ', error: ' + error);
          });
        } else {
          _loggerProxy2.default.logger.info('Meeting:index#setUpLocusInfoMeetingListener --> MEETING_REMOVED_REASON', payload.reason);
          _util2.default.cleanUp(_this12);
          _triggerProxy2.default.trigger(_this12, {
            file: 'meeting/index',
            function: 'setUpLocusInfoMeetingListener'
          }, _constants.EVENTS.DESTROY_MEETING, {
            reason: payload.reason,
            meetingId: _this12.id
          });
        }
      });
    }
    ...
    value: function processMQAData() {
      ...
      _constants.MQA_STATS.DATA_PLACEMENTS.forEach(function (key) {
        if (_this15.mediaQualityMetrics && _this15.mediaQualityMetrics.stats) {
          var sendRecvData = void 0;

          if (key.toLowerCase().endsWith(_constants.STATS.SENDER)) {
            sendRecvData = _this15.mediaQualityMetrics.stats.getSender(key);
          } else if (key.toLowerCase().endsWith(_constants.STATS.RECEIVER)) {
            sendRecvData = _this15.mediaQualityMetrics.stats.getReceiver(key);
          }
          var mqa = void 0;
          ...
          if (mqa) {
            var interval = mqa.getSlice(_this15.config.metrics.mqaMetricsInterval / 1000); // milliseconds -> second based intervals

            _this15.mqaProcessor.process(key, interval);
          }
        }
      });
    }
    ...
    value: function setRemoteStream(pc) {
      ...
      pc.ontrack = function (event) {
        ...
        var trackMediaID = event.transceiver.mid;
        var eventType = null;
        ...
        switch (trackMediaID) {
          case MEDIA_ID.AUDIO_TRACK:
            ...
            eventStream = _util4.default.createMediaStream([event.track]);
            _this16.mediaProperties.setRemoteStream(eventStream);
            break;
          ...
          case MEDIA_ID.SHARE_TRACK:
            if (event.track) {
              ...
              eventStream = _util4.default.createMediaStream([event.track]);
              _this16.mediaProperties.setRemoteShare(eventStream);
            }
            break;
          default:
            {
              _loggerProxy2.default.logger.log('Meeting:index#setRemoteStream --> no matching media track id');
            }
        }
    ...
    value: function setShareStream(pc) {
      ...
      pc.ontrack = function (event) {
        ...
        _this18.mediaProperties.remoteShare = _event$streams[0];

        _triggerProxy2.default.trigger(_this18, {
          file: 'meeting/index',
          function: 'setShareStream:pc.ontrack'
        }, _constants.EVENT_TRIGGERS.MEDIA_READY, {
          type: _constants.EVENT_TYPES.REMOTE_SHARE,
          stream: _this18.mediaProperties.remoteShare
        });
      };
    }
    ...
    value: function setLocalTracks(localStream) {
      if (localStream) {
        ...
        if (audioTrack) {
          ...
          _loggerProxy2.default.logger.log('Meeting:index#setLocalTracks --> Audio settings.', (0, _stringify2.default)(this.mediaProperties.mediaSettings.audio));
        ...
        if (videoTrack) {
          ...
          _loggerProxy2.default.logger.log('Meeting:index#setLocalTracks --> Video settings.', (0, _stringify2.default)(this.mediaProperties.mediaSettings.video));
        }
    ...
    value: function setLocalShareTrack(localShare) {
      ...
      if (localShare) {
        ...
        if (contentTracks) {
          ...
          _loggerProxy2.default.logger.log('Meeting:index#setLocalTracks --> Screen settings.', (0, _stringify2.default)(this.mediaProperties.mediaSettings.screen));
        }

        contentTracks.onended = function () {
          if (_this19.wirelessShare) {
            _this19.leave({ reason: _constants.MEETING_REMOVED_REASON.USER_ENDED_SHARE_STREAMS });
          } else {
            _this19.stopShare();
          }
        };
    ...
    value: function closeLocalStream() {
      ...
      return _media2.default.stopTracks(this.mediaProperties.audioTrack).then(function () {
        return _media2.default.stopTracks(_this20.mediaProperties.videoTrack);
      }).then(function () {
    ...
    value: function closeLocalShare() {
      ...
      return _media2.default.stopTracks(this.mediaProperties.shareTrack).then(function () {
        _triggerProxy2.default.trigger(_this21, {
          file: 'meeting/index',
    ...
    value: function setReconnectListener() {
      ...
      this.webex.internal.mercury.on(_constants.ONLINE, function () {
        _loggerProxy2.default.logger.log('Meeting:index#setReconnectListener --> Web socket online');
      });
    }
    ...
    value: function joinWithMedia() {
      ...
      return this.join(joinOptions).then(function (joinResponse) {
        return _this26.getMediaStreams(mediaSettings, audioVideoOptions).then(function (_ref3) {
        ...
        _loggerProxy2.default.logger.error('Meeting:index#joinWithMedia --> ', error);
    ...
    value: function reconnect(options) {
      ...
      _loggerProxy2.default.logger.log('Meeting:index#reconnect --> attempting to reconnect');
      ...
      try {
        _loggerProxy2.default.logger.info('Meeting:index#reconnect --> Validating reconnect ability.');
        this.reconnectionManager.validate();
      } catch (error) {
        // Unable to reconnect this call
        if (error instanceof _reconnectionInProgress2.default) {
          _loggerProxy2.default.logger.info('Meeting:index#reconnect --> Unable to reconnect, reconnection in progress.');
        } else {
          _loggerProxy2.default.logger.log('Meeting:index#reconnect --> Unable to reconnect.', error);
        }
      ...
      return this.reconnectionManager.reconnect(options).then(function () {
        ...
        _loggerProxy2.default.logger.log('Meeting:index#reconnect --> Meeting reconnect success');
      }).catch(function (error) {
        _triggerProxy2.default.trigger(_this27, {
          file: 'meeting/index',
          function: 'reconnect'
        ...
        _loggerProxy2.default.logger.error('Meeting:index#reconnect --> Meeting reconnect failed', error);
    ...
    value: function join() {
      ...
      _loggerProxy2.default.logger.log('Meeting:index#join --> Joining a meeting');
      ...
      if (options.meetingQuality) {
        if (typeof options.meetingQuality === 'string') {
          if (!_constants.QUALITY_LEVELS[options.meetingQuality]) {
            var errorMessage = 'Meeting:index#join --> ' + options.meetingQuality + ' not defined';
        ...
        if ((0, _typeof3.default)(options.meetingQuality) === 'object') {
          if (!_constants.QUALITY_LEVELS[options.meetingQuality.local] && !_constants.QUALITY_LEVELS[options.meetingQuality.remote]) {
            var _errorMessage = 'Meeting:index#join --> ' + (options.meetingQuality.local || options.meetingQuality.remote) + ' not defined';
      ...
      return _util2.default.joinMeetingOptions(this, options).then(function (join) {
        ...
        _loggerProxy2.default.logger.log('Meeting:index#join --> Success');

        return join;
      }).catch(function (error) {
        _this28.meetingFiniteStateMachine.fail(error);
        _loggerProxy2.default.logger.error('Meeting:index#join --> Failed', error);
    ...
    value: function moveFrom(resourceId) {
      ...
      return _util2.default.joinMeetingOptions(this).then(function (join) {
        return _this30.getMediaStreams({ sendAudio: true, sendVideo: true, sendShare: false }).then(function (_ref5) {
    ...
    value: function addMedia() {
      ...
      _loggerProxy2.default.logger.info(LOG_HEADER + ' Adding Media.');
      ...
      return _util2.default.validateOptions(options).then(function () {
        ...
        _peerConnectionManager2.default.setPeerConnectionEvents(_this31);

        return _this31.preMedia(localStream, localShare, mediaSettings);
      }).then(function () {
        return _media2.default.attachMedia(_this31.mediaProperties, {
          meetingId: _this31.id,
          remoteQualityLevel: _this31.mediaProperties.remoteQualityLevel
        }).then(function (peerConnection) {
          _loggerProxy2.default.logger.info(LOG_HEADER + ' PeerConnection Received from attachMedia ' + peerConnection);
          ...
          if (_this31.config.stats.enableStatsAnalyzer) {
            _this31.statsAnalyzer = new _statsAnalyzer2.default(_this31.config.stats);
            _this31.statsAnalyzer.on(_constants.EVENT_TRIGGERS.MEDIA_QUALITY, function (res) {
              // TODO:  might have to send the same event to the developer
              _metrics2.default.postEvent({ event: _config.eventType.MEDIA_QUALITY, meetingId: _this31.correlationId, data: { intervalData: res } });
            });
            _this31.statsAnalyzer.on(_constants.EVENT_TRIGGERS.MEETING_HIGH_PACKETLOSS, function (res) {
              if (res.mediaType === _constants.VIDEO) {
                if (_this31.config.stats.autoDowngradeEnabled && _this31.canUpdateMedia()) {
                  _loggerProxy2.default.logger.log(LOG_HEADER + ' High Packet Loss Detected, Adjusting Local Video Quality');
                  if (_this31.mediaProperties.localQualityLevel === _constants.QUALITY_LEVELS.HIGH) {
                    _this31.setLocalVideoQuality(_constants.QUALITY_LEVELS.MEDIUM);
                  } else if (_this31.mediaProperties.localQualityLevel === _constants.QUALITY_LEVELS.MEDIUM) {
                    _this31.setLocalVideoQuality(_constants.QUALITY_LEVELS.LOW);
                  }
                }
                _triggerProxy2.default.trigger(_this31, {
                  file: 'meeting/index',
                  function: 'addMedia'
            ...
            _this31.statsAnalyzer.startAnalyzer(peerConnection);
          }
        }).catch(function (err) {
          _loggerProxy2.default.logger.error(LOG_HEADER + ' Error joining the call on roap initialization, ', err);
          throw err;
        }).then(function () {
          ...
          return logRequest(_this31.roap.sendRoapMediaRequest({
    ...
    value: function updateMedia() {
      ...
      return _util2.default.validateOptions(options).then(function () {
        ...
        return _media2.default.updateMedia(_this32.mediaProperties, {
          meetingId: _this32.id,
          remoteQualityLevel: _this32.mediaProperties.remoteQualityLevel
        }).then(function (peerConnection) {
          _loggerProxy2.default.logger.info(LOG_HEADER + ' PeerConnection received from updateMedia, ' + peerConnection);
          ...
          _loggerProxy2.default.logger.error(LOG_HEADER + ' Error updatedMedia, ', err);
          throw err;
        }).then(function () {
    ...
    value: function leave() {
      ...
      _loggerProxy2.default.logger.log('Meeting:index#leave --> Leaving a meeting');
      ...
      return _util2.default.leaveMeeting(this, options).then(function (leave) {
        ...
        _loggerProxy2.default.logger.log('Meeting:index#leave --> LEAVE REASON ', leaveReason);

        return leave;
      }).catch(function (error) {
        _this38.meetingFiniteStateMachine.fail(error);
        _loggerProxy2.default.logger.error('Meeting:index#leave --> Failed to leave ', error);
    ...
    value: function changeVideoLayout(layoutType) {
      ...
      if (this.mediaProperties.mediaDirection.receiveVideo === true && this.mediaProperties.remoteStream) {
        if (_constants.LAYOUT_TYPES.includes(layoutType)) {
          return this.meetingRequest.changeVideoLayout({
            locusUrl: this.locusInfo.self.url,
            deviceUrl: this.deviceUrl,
            layoutType: layoutType
          }).then(function (response) {
            if (response && response.body && response.body.locus) {
              _this41.locusInfo.onFullLocus(response.body.locus);
            }
          });
        }
    ...
    value: function setLocalVideoQuality(level) {
      ...
      if (!this.mediaProperties.mediaDirection.sendVideo) {
        var _errorMessage2 = 'Meeting:index#setLocalVideoQuality --> unable to change video quality, sendVideo is disabled';

        _loggerProxy2.default.logger.error(_errorMessage2);

        return _promise2.default.reject(new Error(_errorMessage2));
      }

      // If level is already the same, don't do anything
      if (level === this.mediaProperties.localQualityLevel) {
        _loggerProxy2.default.logger.warn('Meeting:index#setLocalQualityLevel --> Quality already set to ' + level);
      ...
      return this.getMediaStreams(mediaDirection, _constants.VIDEO_RESOLUTIONS[level]).then(function (_ref7) {
        var _ref8 = (0, _slicedToArray3.default)(_ref7, 1),
            localStream = _ref8[0];

        return _this42.updateVideo({
          sendVideo: true,
          receiveVideo: true,
          stream: localStream
        });
    ...
    value: function setRemoteQualityLevel(level) {
      _loggerProxy2.default.logger.log('Meeting:index#setRemoteQualityLevel --> Setting quality to ' + level);

      if (!_constants.QUALITY_LEVELS[level]) {
        var errorMessage = 'Meeting:index#setRemoteQualityLevel --> ' + level + ' not defined';
      ...
      if (!this.mediaProperties.mediaDirection.receiveAudio && !this.mediaProperties.mediaDirection.receiveVideo) {
        var _errorMessage3 = 'Meeting:index#setRemoteQualityLevel --> unable to change remote quality, receiveVideo and receiveAudio is disabled';

        _loggerProxy2.default.logger.error(_errorMessage3);

        return _promise2.default.reject(new Error(_errorMessage3));
      }

      // If level is already the same, don't do anything
      if (level === this.mediaProperties.remoteQualityLevel) {
        _loggerProxy2.default.logger.warn('Meeting:index#setRemoteQualityLevel --> Quality already set to ' + level);
    ...
    value: function setMeetingQuality(level) {
      ...
      _loggerProxy2.default.logger.log('Meeting:index#setMeetingQuality --> Setting quality to ' + level);

      if (!_constants.QUALITY_LEVELS[level]) {
        var errorMessage = 'Meeting:index#setMeetingQuality --> ' + level + ' not defined';
      ...
      if (level === this.mediaProperties.localQualityLevel && level === this.mediaProperties.remoteQualityLevel) {
        _loggerProxy2.default.logger.warn('Meeting:index#setMeetingQuality --> Quality already set to ' + level);

        return _promise2.default.resolve();
      }

      // Determine the direction of our current media
      var _mediaProperties$medi = this.mediaProperties.mediaDirection,
          receiveAudio = _mediaProperties$medi.receiveAudio,
          receiveVideo = _mediaProperties$medi.receiveVideo,
          sendVideo = _mediaProperties$medi.sendVideo;


      return (sendVideo ? this.setLocalVideoQuality(level) : _promise2.default.resolve()).then(function () {
        return receiveAudio || receiveVideo ? _this43.setRemoteQualityLevel(level) : _promise2.default.resolve();
      }).catch(function (error) {
        // From troubleshooting it seems that the stream itself doesn't change the max-fs if the peer connection isn't stable
        _this43.mediaProperties.setLocalQualityLevel(previousLevel.local);
        _this43.mediaProperties.setRemoteQualityLevel(previousLevel.remote);

        _loggerProxy2.default.logger.error('Meeting:index#setMeetingQuality --> ' + error.message);
    ...
    */
    ...
    value: function shareScreen() {
      ...
      _loggerProxy2.default.logger.log('Meeting:index#shareScreen --> Getting local share');

      var shareConstraints = (0, _extends3.default)({
        sendShare: true,
        sendAudio: false
      }, options);

      return _media2.default.getDisplayMedia(shareConstraints, this.config).then(function (shareStream) {
        _this44.updateShare({
          sendShare: true,
          receiveShare: _this44.mediaProperties.mediaDirection.receiveShare,
          stream: shareStream
        });
      }).catch(function (error) {
        ...
        _metrics2.default.sendOperationalMetric(metricName, data, metadata);
        throw new _media4.default('Unable to retrieve display media stream', error);
      });
    }
  }]);
  return Meeting;
