...
var LocusInfo = function (_EventsScope) {
  ...
  (0, _createClass3.default)(LocusInfo, [{
    ...
    value: function parse(meeting, data) {
      ...
      _loggerProxy2.default.logger.info('Locus-info:index#parse --> received locus data: ' + eventType);
    ...
    value: function onFullLocus(locus, eventType) {
      if (!locus) {
        _loggerProxy2.default.logger.error('Locus-info:index#onFullLocus --> object passed as argument was invalid, continuing.');
      }
    ...
    value: function isMeetingActive() {
      if (this.parsedLocus.fullState.type === _constants._CALL_) {
        ...
        if (this.fullState && this.fullState.state === _constants.LOCUS.STATE.INACTIVE) {
          ...
          _loggerProxy2.default.logger.warn('Locus-info:index#isMeetingActive --> Call Ended, locus state is inactive.');
        ...
        _loggerProxy2.default.logger.warn('Locus-info:index#isMeetingActive --> Meeting Type is unknown.');
      }
    }
    ...
    key: 'reportParticipantChanges',
    value: function reportParticipantChanges() {
      var _this2 = this;

      // Used to find a participant from a given collection.
      var findParticipant = function findParticipant() {
        ...
        return collection.find(function (participant) {
          return participant.person.id === person.id;
        }) || {};
      };

      // Used to setup a changes object for metrics.
      var determineChanges = function determineChanges(participant, deltas) {
        return (0, _keys2.default)(deltas).reduce(function (states, key) {
          var changes = {
            target: undefined,
            state: undefined
          };
          ...
          switch (key) {
            case _constants.AUDIO_STATUS:
              changes.target = _constants.PARTICIPANT_DELTAS.TARGETS.AUDIO;
              break;

            case _constants.VIDEO_STATUS:
              changes.target = _constants.PARTICIPANT_DELTAS.TARGETS.VIDEO;
              break;
          ...
          switch (participant.status[key]) {
            case _constants._INACTIVE_:
              changes.state = _constants.PARTICIPANT_DELTAS.STATES.DISABLED;
              break;

            case _constants._RECEIVE_ONLY_:
              changes.state = _constants.PARTICIPANT_DELTAS.STATES.MUTED;
              break;

            case _constants._SEND_RECEIVE_:
              changes.state = _constants.PARTICIPANT_DELTAS.STATES.UNMUTED;
              break;

            default:
              changes.state = _constants.PARTICIPANT_DELTAS.STATES.UNKNOWN;
          }
      ...
      this.deltaParticipants.forEach(function (changedParticipant) {
        var participant = findParticipant(changedParticipant.person, _this2.participants);

        var changes = determineChanges(participant, changedParticipant.delta);

        if (changes.every(function (change) {
          return change.state === _constants.PARTICIPANT_DELTAS.STATES.DISABLED;
        })) {
          var meeting = _this2.webex.meetings.meetingCollection.get(_this2.meetingId);

          _metrics2.default.sendOperationalMetric(_constants.METRICS_OPERATIONAL_MEASURES.MEETING_MEDIA_RELEASE, {
    ...
    value: function updateParticipantDeltas() {
      ...
      var generateDelta = function generateDelta() {
        ...
        // Clean the object
        (0, _keys2.default)(deltas).forEach(function (key) {
          if (deltas[key] !== true) {
            delete deltas[key];
          }
        });
      ...
      this.deltaParticipants = participants.reduce(function (collection, participant) {
        var existingParticipant = findParticipant(participant, _this3.participants || []) || {};

        var delta = generateDelta(existingParticipant.status, participant.status);

        var changed = (0, _keys2.default)(delta).length > 0;
      ...
      if (self && !(0, _isEqual3.default)(this.self, self)) {
        ...
        if (parsedSelves.updates.isMediaInactiveOrReleased) {
          this.emitScoped({
            file: 'locus-info',
            function: 'updateSelf'
          }, _constants.LOCUSINFO.EVENTS.DISCONNECT_DUE_TO_INACTIVITY, { reason: self.reason });
        }
        ...
        if (parsedSelves.updates.isMediaInactive) {
          this.emitScoped({
            file: 'locus-info',
            function: 'updateSelf'
          }, _constants.LOCUSINFO.EVENTS.MEDIA_INACTIVITY, _selfUtils2.default.getMediaStatus(self.mediaSessions));
        }
