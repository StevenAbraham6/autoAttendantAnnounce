...
Media.reconnectMedia = function (peerConnection, _ref) {
  ...
  return _peerConnectionManager2.default.createOffer(peerConnection, { meetingId: meetingId, remoteQualityLevel: remoteQualityLevel });
};
...
Media.attachMedia = function (mediaProperties, _ref2) {
  ...
  peerConnection.onnegotiationneeded = function (event) {
    _loggerProxy2.default.logger.info('Media:index#attachMedia --> onnegotiationneeded#PeerConnection: ' + event);
  };
...
Media.updateMedia = function (mediaProperties, _ref3) {
  ...
  peerConnection.onnegotiationneeded = function (event) {
    _loggerProxy2.default.logger.info('Media:index#updateMedia --> onnegotiationneeded#PeerConnection: ' + event);
  };
...
Media.setTrackOnTransceiver = function (transceiver, options) {
  ...
  try {
    ...
    _loggerProxy2.default.logger.error('Media:index#setTrackOnTransceiver --> ' + e);
    throw e;
  }
};
...
Media.getDisplayMedia = function (options) {
  ...
  var _Config$meetings = _config2.default.meetings,
  ...
  if (hasCustomConstraints) {
    ...
    shareConstraints = (0, _extends3.default)({}, shareConstraints, {
  ...
  var getDisplayMediaParams = { video: options.sendShare ? shareConstraints : false };

  // safari doesn't support sending screen share audio
  // https://developer.mozilla.org/en-US/docs/Web/API/MediaDevices/getDisplayMedia
  if (options.sendAudio && _bowser2.default.name.toLowerCase() !== 'safari') {
...
Media.getMedia = function (audio, video, config) {
  var defaultWidth = { ideal: config.resolution.idealWidth, max: config.resolution.maxWidth };
  var defaultHeight = { ideal: config.resolution.idealHeight, max: config.resolution.maxHeight };
  var mediaConfig = {
    audio: audio,
    // TODO: Remove temporary workaround once Firefox fixes low constraint issues
    // eslint-disable-next-line no-nested-ternary
    video: video ? _bowser2.default.firefox && video.width && video.width.max === 320 ? {
  ...
  return navigator.mediaDevices.getUserMedia(mediaConfig).catch(function (err) {
    ...
    _loggerProxy2.default.logger.error(logPath + ' failed - ' + err + ' (' + err.constraint + ')');
    throw err;
  });
};
