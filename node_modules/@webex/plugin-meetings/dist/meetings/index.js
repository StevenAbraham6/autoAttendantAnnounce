...
var _request = require('../common/logs/request');
...
var _request3 = require('../meetings/request');
...
var Meetings = function (_WebexPlugin) {
  ...
  function Meetings() {
     ...
     * @memberof Meetings
     */
    _this.request = new _request4.default({}, { parent: _this.webex });
    /**
     ...
     * @memberof Meetings
     */
    _this.loggerRequest = new _request2.default({ webex: _this.webex });
    ...
    _util2.default.triggerH264Codec();
    _metrics2.default.initialSetup(_this.meetingCollection, _this.webex);
    return _this;
  }
  ...
  (0, _createClass3.default)(Meetings, [{
    ...
    value: function handleLocusEvent(data) {
      ...
      if (!meeting) {
        ...
        if (data.locus && data.locus.fullState && data.locus.fullState.state === _constants.LOCUS.STATE.INACTIVE) {
          ...
          _loggerProxy2.default.logger.warn('Meetings:index#handleLocusEvent --> Locus event received for meeting, after it was ended.');
        ...
        this.create(data.locus, _constants._LOCUS_ID_).then(function (newMeeting) {
          ...
          if (_this2.getMeetingByType(_constants._ID_, meeting.id)) {
            ...
            _loggerProxy2.default.logger.warn('Meetings:index#handleLocusEvent --> Created and destroyed meeting object before sending an event');
          }
        });
      } else {
        _metrics2.default.postEvent({ event: _config.eventType.NOTIFICATION_RECEIVED, meeting: meeting, data: { trigger: _config.trigger.MERCURY_EVENT } });
    ...
    value: function register() {
      ...
      if (!this.webex.canAuthorize) {
        _loggerProxy2.default.logger.error('Meetings:index#register --> ERROR, Unable to register, SDK cannot authorize');

        return _promise2.default.reject(new Error('SDK cannot authorize'));
      }

      if (this.registered) {
        _loggerProxy2.default.logger.info('Meetings:index#register --> INFO, Meetings plugin already registered');
      ...
      return this.webex.internal.device.register().then(function () {
        ...
        _loggerProxy2.default.logger.error('Meetings:index#register --> ERROR, Unable to register, ' + error.message);
    ...
    value: function unregister() {
      ...
      if (!this.registered) {
        _loggerProxy2.default.logger.info('Meetings:index#unregister --> INFO, Meetings plugin already unregistered');
    ...
    value: function destroy(meeting, reason) {
      ...
      if (this.config.autoUploadLogs) {
        _loggerProxy2.default.logger.info('Meetings:index#destroy --> Meeting destroyed, uploading logs');
        var callStart = meeting.locusInfo && meeting.locusInfo.fullState ? meeting.locusInfo.fullState.lastActive : undefined;
        ...
        this.uploadLogs({
          ...
          _loggerProxy2.default.logger.info('Meetings:index#destroy --> Upload logs for meeting completed.', uploadResult);
          _triggerProxy2.default.trigger(_this7, {
            file: 'meetings',
            function: 'destroy'
          ...
          _loggerProxy2.default.logger.error('Meetings:index#destroy --> Unable to upload logs for meeting', uploadError);
          _triggerProxy2.default.trigger(_this7, {
            file: 'meetings',
            function: 'destroy'
    ...
    value: function create(destination) {
      ...
      .catch(function (error) {
        _loggerProxy2.default.logger.info('Meetings:index#create --> INFO, unable to determine info options: ' + error.message);
      }).then(function () {
        var options = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};

        // Normalize the destination.
        var targetDest = options.wasHydraPerson ? options.destination : destination;

        // Attempt to collect the meeting if it exists.
        var meeting = _this8.meetingCollection.getByKey(_constants.SIP_URI, targetDest);

        // Validate if a meeting was found.
        if (!meeting) {
          // Create a meeting based on the normalized destination and type.
          return _this8.createMeeting(targetDest, type).then(function (createdMeeting) {
            // If the meeting was successfully created.
            if (createdMeeting && createdMeeting.on) {
              // Create a destruction event for the meeting.
              createdMeeting.on(_constants.EVENTS.DESTROY_MEETING, function (payload) {
                _this8.destroy(createdMeeting, payload.reason);
              });
            } else {
              _loggerProxy2.default.logger.error('Meetings:index#create --> ERROR, meeting does not have on method, will not be destroyed, meeting cleanup impossible for meeting: ' + meeting);
            }

            // Return the newly created meeting.
            return _promise2.default.resolve(createdMeeting);
          });
        }
          ...
          while (1) {
            switch (_context.prev = _context.next) {
              ...
              case 10:
                ...
                _loggerProxy2.default.logger.info('Meetings:index#createMeeting --> Info Unable to fetch meeting info for ' + destination + '.');
                _loggerProxy2.default.logger.info('Meetings:index#createMeeting --> Info assuming this destination is a 1:1 or wireless share');
                _loggerProxy2.default.logger.debug('Meetings:index#createMeeting --> Debug ' + _context.t0 + ' fetching /meetingInfo for creation.');
      ...
      return this.request.getActiveMeetings().then(function (locusArray) {
        ...
        if (locusArray.loci && locusArray.loci.length > 0) {
          locusArray.loci.forEach(function (locus) {
            ...
            _this9.handleLocusEvent({
              locus: locus,
              locusUrl: locus.url
            });
          });
        }
        if ((0, _keys2.default)(_this9.meetingCollection.getAll()).length > 0) {
          // Some time the mercury event is missed after mercury reconnect
          // if sync returns no locus then clear all the meetings
          (0, _forEach3.default)(_this9.meetingCollection.getAll(), function (meeting) {
            if (!activeLocusUrl.includes(meeting.locusUrl)) {
              _this9.destroy(meeting, _constants.MEETING_REMOVED_REASON.NO_MEETINGS_TO_SYNC);
            }
          });
        }
      });
